...
class FPN(BaseNeck):
    def __init__(
        ...
        self.upsample = nn.Upsample(scale_factor=2. if top_down else 0.5, mode=interpolation_mode)
...
class PAN(BaseNeck):
    def __init__(
        ...
        super().__init__()
        self.top_down = FPN(in_channels, out_channels, fuse_fn=fuse_fn, block=block, interpolation_mode=interpolation_mode)
        self.bottom_up = FPN([out_channels] * len(in_channels), out_channels, fuse_fn=fuse_fn, block=block, interpolation_mode=interpolation_mode)
...
class BiFPN(BaseNeck):    
    def __init__(
        ...
        self.layers = nn.Sequential(*[BiFPNLayer(len(in_channels), out_channels, block=block, interpolation_mode=interpolation_mode, eps=eps) for _ in range(num_layers)])
